...
from sentence_embedder import sentence_processor
...
def load_biomedical_hf(data_path='datasets/Biomedical',
                       ...
                       scaler: Union[Literal['MinMax', 'Standard'], None]=None,
                       norm: Union[Literal['l2', 'l1', 'max'], None]=None):
    embedding = np.load(data_path+'/HuggingFace/output_bio.npy')

    if norm is not None:
        processed_data = normalize(embedding, norm=norm)
    
    if scaler == 'MinMax':
        scaler = MinMaxScaler()
        processed_data = scaler.fit_transform(embedding)
        print('MinMax scaling completed...')
    elif scaler == 'Standard':
        scaler = StandardScaler()
        processed_data = scaler.fit_transform(embedding)
...
# JoSE Embeddings
def load_stackoverflow_jose(data_path='datasets/stackoverflow',
                            transform: Optional[Literal['SIF', None]]=None,
                            scaler: Optional[Literal['MinMax', 'Standard']]=None,
                            norm: Optional[Literal['l2', 'l1', 'max']]=None):
    print("performing Jose embedding...")
    with open(data_path+'/jose/jose_sof_doc_d20.txt', 'r') as f:
        lines = f.readlines()

    # Get the total number of embeddings and the dimension from the first line
    total_embeddings, dimension = map(int, lines[0].strip().split())

    # Initialize an empty matrix with the given dimensions
    embeddings_matrix = np.zeros((total_embeddings, dimension))

    # Fill the matrix with the embeddings
    for line in lines[1:]:
        index_and_embedding = line.strip().split()
        index = int(index_and_embedding[0])
        embedding = list(map(float, index_and_embedding[1:]))
        embeddings_matrix[index] = embedding

    y = read_label(dataset=data_path)

    return embeddings_matrix, y

def load_biomedical_jose(data_path='datasets/Biomedical',
                         transform: Optional[Literal['SIF', None]]=None,
                         scaler: Optional[Literal['MinMax', 'Standard']]=None,
                         norm: Optional[Literal['l2', 'l1', 'max']]=None):
    with open(data_path+'/jose/jose_bio_doc_d20.txt', 'r') as f:
        lines = f.readlines()
...
def load_searchsnippet_jose(data_path='datasets/SearchSnippets',
                            transform: Optional[Literal['SIF', None]]=None,
                            scaler: Optional[Literal['MinMax', 'Standard']]=None,
                            norm: Optional[Literal['l2', 'l1', 'max']]=None):
    with open(data_path+'/jose/jose_ssp_doc_d20.txt', 'r') as f:
        lines = f.readlines()
...
def load_stackoverflow_hfemb(data_path='datasets/stackoverflow',
                             transform: Optional[Literal['SIF', None]]=None,
                             scaler: Optional[Literal['MinMax', 'Standard']]=None,
                             norm: Optional[Literal['l2', 'l1', 'max']]=None):
    with open(data_path+'/HuggingFace/embeddings.txt', 'r') as file:
        embeddings = []
        for line in file:
...
def load_biomedical_hfemb(data_path='datasets/Biomedical',
                          transform: Optional[Literal['SIF', None]]=None,
                          scaler: Optional[Literal['MinMax', 'Standard']]=None,
                          norm: Optional[Literal['l2', 'l1', 'max']]=None):
    with open(data_path+'/HuggingFace/embeddings.txt', 'r') as file:
...
def load_searchsnippet_hfemb(data_path='datasets/SearchSnippets',
                             transform: Optional[Literal['SIF', None]]=None,
                             scaler: Optional[Literal['MinMax', 'Standard']]=None,
                             norm: Optional[Literal['l2', 'l1', 'max']]=None):
    with open(data_path+'/HuggingFace/embeddings.txt', 'r') as file:
...
###############################################################################
# TF-IDF Embeddings
def load_searchsnippet_tfidf(data_path='datasets/stackoverflow',
                             transform: Optional[Literal['SIF', None]]=None,
                             scaler: Optional[Literal['MinMax', 'Standard']]=None,
                             norm: Optional[Literal['l2', 'l1', 'max']]=None):

    sentences = sentence_processor(data_path + '/SearchSnippets.txt')
...
def load_stackoverflow_tfidf(data_path='datasets/SearchSnippets',
                             transform: Optional[Literal['SIF', None]]=None,
                             scaler: Optional[Literal['MinMax', 'Standard']]=None,
                             norm: Optional[Literal['l2', 'l1', 'max']]=None):

    sentences = sentence_processor(data_path + '/title_StackOverflow.txt')
    print("Extracting features from the training dataset using a sparse vectorizer")
...
def load_biomedical_tfidf(data_path='datasets/Biomedical',
                             transform: Optional[Literal['SIF', None]]=None,
                             scaler: Optional[Literal['MinMax', 'Standard']]=None,
                             norm: Optional[Literal['l2', 'l1', 'max']]=None):

    sentences = sentence_processor(data_path + '/Biomedical.txt')
    print("Extracting features from the training dataset using a sparse vectorizer")
...
def load_data(dataset='datasets/stackoverflow', 
              word_emb: Union[Literal['Word2Vec', 'Jose', 'Glove', 'HuggingFace', 'TF-IDF'], None]=None,
              transform: Union[Literal['SIF'], None]=None,
              scaler: Union[Literal['MinMax', 'Standard'], None]=None,
              norm: Union[Literal['l2', 'l1', 'max'], None]=None):
    ...
    if dataset.endswith('stackoverflow'):
        if word_emb == "Word2Vec":
            ...
            print('Performing HuggingFace embedding...')
            return load_stackoverflow_hfemb(data_path=dataset,
                                            transform=transform, 
                                            scaler=scaler, 
                                            norm=norm)
        elif word_emb == "TF-IDF":
            print('Performing TF-IDF representation...')
            return load_stackoverflow_tfidf(data_path=dataset,
                                            transform=transform, 
                                            scaler=scaler, 
                                            norm=norm)
        else:
        ...
        if word_emb == "Word2Vec":
            ...
            return load_biomedical_hfemb(data_path=dataset,
                                         transform=transform, 
                                         scaler=scaler, 
                                         norm=norm)
        elif word_emb == "TF-IDF":
            return load_biomedical_tfidf(data_path=dataset,
                                         transform=transform, 
                                         scaler=scaler, 
                                         norm=norm)
        else:
        ...
        if word_emb == "Word2Vec":
            ...
            return load_searchsnippet_jose(data_path=dataset,
                                           transform=transform, 
                                           scaler=scaler, 
                                           norm=norm)
        elif word_emb == "HuggingFace":
            return load_searchsnippet_hfemb(data_path=dataset,
                                            transform=transform, 
                                            scaler=scaler, 
                                            norm=norm)
        elif word_emb == "TF-IDF":
            return load_searchsnippet_tfidf(data_path=dataset,
                                            transform=transform, 
                                            scaler=scaler, 
                                            norm=norm)
    else:
